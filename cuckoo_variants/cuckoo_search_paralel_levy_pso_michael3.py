...
def cuckoo_search(n=None, nd=None, Lb=None, Ub=None, pa=None):
	...
	pa = np.linspace(0.1,0.9,8)
	step = 1
	c1 = np.linspace(0,1.5,8)
	c2 = 0.01
	all_para = list(itertools.product(pa,c1))
	...
	n_jobs = mp.cpu_count()
	for i in range(100):
		a = Parallel(n_jobs=n_jobs)(delayed(single_cuckoo_search)(nests[j],fitness[j],Lb,Ub,all_para[j][0],step,all_para[j][1],c2,pbests[j]) for j in range(64))
		best_nest, fmin, nests, fitness, N_iter, done, pbests = zip(*a)
		if True in done:
			ind = np.argmin(fmin)
		...
		if i != 0:
			...
			total_best_min = fmin[ind]
			total_best_nest = best_nest[ind]
			print total_best_nest, total_best_min, ind
...
def single_cuckoo_search(nest,fitness,Lb,Ub,pa,step,c1,c2, pbests):
	...
	return bestnest, fmin, nest, fitness, N_iter, done, pbest
